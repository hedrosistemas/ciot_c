...
#ifndef __CIOT__ERR__H__
#define __CIOT__ERR__H__

#include "ciot/proto/v1/errors.pb-c.h"
#include "ciot_log.h"

typedef Ciot__Err ciot_err_t;
...
#define CIOT_ERR_NULL_CHECK(x)                        \
    if (x == NULL)                                    \
    {                                                 \
        CIOT_ERR_LOG("ciot_err", CIOT__ERR__NULL_ARG); \
        return CIOT__ERR__NULL_ARG;                    \
    }

#define CIOT_ERR_INDEX_CHECK(index, min, max) \
    if (index < min)                          \
        return CIOT__ERR__INVALID_INDEX;       \
    if (index > max)                          \
        return CIOT__ERR__INVALID_INDEX;
...
#define CIOT_ERR_STATE_CHECK(current, expected)            \
    if (current != expected)                               \
    {                                                      \
        CIOT_ERR_LOG("ciot_err", CIOT__ERR__INVALID_STATE); \
...
#define CIOT_ERR_EMPTY_STRING_CHECK(x)                       \
    if (x[0] == '\0')                                        \
    {                                                        \
        /*CIOT_ERR_LOG("ciot_err", CIOT__ERR__INVALID_ARG);*/ \
