...
#define PKT_SET_PRN_PARAM_LEN (3)
...
// static ciot_err_t ciot_dfu_nrf_encode_and_send(ciot_dfu_nrf_t self, uint8_t *data, int size);
static ciot_err_t ciot_dfu_nrf_timeout_check(int timeout);
...
static ciot_err_t ciot_iface_send_data(ciot_iface_t *iface, uint8_t *data, int size)
{
    ...
    return destiny->send_data(destiny, data, size);
}
...
ciot_err_t ciot_dfu_nrf_process_req(ciot_dfu_nrf_t self, ciot_dfu_req_t *req)
{
    ...
    switch (req->type)
    {
    case CIOT__DFU_REQ_TYPE__DFU_REQ_SEND_FIRMWARE:
        CIOT_LOGI(TAG, "DFU_REQ_SEND_FIRMWARE");
        base->iface.req_status.state = CIOT_IFACE_REQ_STATE_IDLE;
        return ciot_dfu_nrf_send_firmware(self);        
    default:
        break;
    }
...
static ciot_err_t ciot_dfu_nrf_write(ciot_dfu_nrf_t self)
{
    ...
    if(self->state == CIOT_DFU_NRF_STATE_SEND_PING)
    {
        ...
        ciot_dfu_nrf_timeout_check(TIMEOUT_RESET);
    ...
    return ciot_dfu_nrf_timeout_check(CIOT_DFU_NRF_TIMEOUT_SEC);
}
...
static ciot_err_t ciot_dfu_nrf_event_handler(ciot_iface_t *sender, ciot_iface_event_t *event, void *args)
{
    ...
    if(event->type == CIOT_IFACE_EVENT_DATA)
    {
        ...
        //             ciot_dfu_nrf_process_data(self, self->slip.buffer, self->slip.handle.current_index);
        ...
        // else
        // {
            ciot_dfu_nrf_process_data(self, event->data, event->size);
        // }
    }
