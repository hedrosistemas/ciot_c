...
ciot_err_t ciot_http_client_init(ciot_http_client_t self)
{
    ...
    ciot__http_client_data__init(&base->data);
    ciot__http_client_cfg__init(&base->cfg);
    ciot__http_client_status__init(&base->status);
    ...
    base->iface.info.type = CIOT__IFACE_TYPE__IFACE_TYPE_HTTP_CLIENT;
...
static ciot_err_t ciot_iface_get_data(ciot_iface_t *iface, ciot_msg_t *msg)
{
    ...
    self->iface.data.http_client = &self->data;
