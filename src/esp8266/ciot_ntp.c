...
#if CIOT_CONFIG_FEATURE_NTP && defined(CIOT_TARGET_ESP8266)
...
ciot_err_t ciot_ntp_start(ciot_ntp_t self, ciot_ntp_cfg_t *cfg)
{
    ...
    for (size_t i = 0; i < CIOT_CONFIG_NTP_SERVERS_COUNT; i++)
    {
        if(self->cfg.server[i][0] != '\0')
        {
            ...
            sntp_setservername(i, self->cfg.server[i]);
        }
    }
    ...
    sntp_setoperatingmode(self->cfg.op_mode);
    sntp_set_sync_interval(self->cfg.sync_interval);
    sntp_set_sync_mode(self->cfg.sync_mode);
