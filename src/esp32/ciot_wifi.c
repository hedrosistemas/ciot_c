...
#if CIOT_CONFIG_FEATURE_WIFI && defined(CIOT_TARGET_ESP32)
...
static ciot_err_t ciot_wifi_tcp_event_handler(void *sender, ciot_iface_event_t *iface_event, void *args);
...
ciot_wifi_t ciot_wifi_new(void *handle)
{
    ...
    ESP_ERROR_CHECK(esp_event_handler_register(WIFI_EVENT, ESP_EVENT_ANY_ID, ciot_wifi_event_handler, self));
    return self;
}
